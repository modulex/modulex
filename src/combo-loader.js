...
// =>
// editor/plugin/??x,b
function getUriConsiderCommonPrefix(
  commonPrefix,
  currentComboUris,
  basePrefix,
  comboPrefix,
...
mix(ComboLoader.prototype, {
   ...
   */
  calculate(unloadedMods, errorList, stack, cache, ret) {
    ...
    for (i = 0; i < unloadedMods.length; i++) {
      ...
      self.calculate(
        mod.getNormalizedRequiredModules(),
        errorList,
        stack,
        cache,
        ret,
  ...
  getComboUris(mods) {
    ...
    function processSamePrefixUriMods(type, basePrefix, sendMods) {
      ...
      function getSentUri() {
        return getUriConsiderCommonPrefix(
          commonPrefix,
          currentComboUris,
          basePrefix,
          comboPrefix,
      ...
      for (var i = 0; i < sendMods.length; i++) {
        ...
        if (
          currentComboUris.length > maxFileNum ||
          getSentUri().length > maxUriLength
        ) {
