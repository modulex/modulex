var run = function(combine) {
  describe(
    'loader should report error ' + (combine ? 'at combo mode' : ''),
    function() {
      var mx = modulex;
      ...
      it('should works', function(done) {
        require.config({
          packages: {
            report: {
              base:
                '/tests/specs/error-report/' + (combine ? 'combo' : 'simple'),
            },
          },
        });
        ...
        Q.all([r1.promise, r2.promise]).then(function() {
          ...
          try {
            ...
            expect(error2[0].id).to.be.equal(
              combine ? 'report/s3' : 'report/s4',
            );
