...
describe('modules and groups', function() {
  ...
  it('combo packages', function() {
    ...
    var r = l.calculate(Module.createModules(['pkg-a/a', 'pkg-c/c']));
    var c = l.getComboUris(r);
    var js = c.js;
    expect(js.length).to.be.equal(2);
    expect(js[0].uri.substring(js[0].uri.indexOf('??'))).to.be.equal('??c.js');
    expect(js[1].uri.substring(js[1].uri.indexOf('??'))).to.be.equal(
      '??pkg-a/a.js,pkg-b/b.js',
    );
  });
  ...
  it('combo packages which have no combo prefix', function() {
    ...
    var r = l.calculate(Module.createModules(['pkg-a/a', 'test/x']));
    var c = l.getComboUris(r);
    var js = c.js;
    expect(js.length).to.be.equal(2);
    expect(js[0].uri.substring(js[0].uri.indexOf('??'))).to.be.equal('??a.js');
  ...
  it('combo packages with different charset', function() {
    ...
    var r = l.calculate(Module.createModules(['pkg-a/a']));
    var c = l.getComboUris(r);
    var js = c.js;
    expect(js.length).to.be.equal(2);
    expect(js[0].uri.substring(js[0].uri.indexOf('??'))).to.be.equal('??a.js');
    expect(js[1].uri.substring(js[1].uri.indexOf('??'))).to.be.equal('??b.js');
  });
  ...
  it('can perform 3 package combo', function() {
    ...
    expect(js[0].uri.substring(js[0].uri.indexOf('??'))).to.be.equal(
      '??pkg-a/a.js,pkg-b/b.js,pkg-c/c.js',
    );
  });
  ...
  it('can skip cross origin package combo', function() {
    var uri =
      'http://' +
      location.hostname +
      ':9999/src/loader/tests/specs/packages-groups';
    require.config({
      group: 'my',
      packages: {
        'pkg-a': {
    ...
    var js = c.js;
    expect(js[0].uri).to.be.equal(
      'http://' +
        location.hostname +
        ':9999/src/loader/tests/specs/packages-groups/pkg-b/??b.js',
    );
    expect(js[1].uri).to.be.equal(
      'http://' +
        location.host +
        '/tests/specs/packages-groups/??pkg-a/a.js,pkg-c/c.js',
    );
  });
});
