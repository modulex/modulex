...
describe('ComboLoader', function() {
  ...
  it('should calculate rightly', function() {
    ...
    var r;
    r = l.calculate(Module.createModules(['test/a', 'test/h']));
    var c = l.getComboUris(r);
    expect(c.js[0].uri).to.be.equal(
      'http://a/??a.js,b.js,d.js,f.js,g.js,e.js,c.js,h.js,m.js',
    );
  });
  ...
  it('should trunk uri by comboMaxFileNum config rightly', function() {
    ...
    var r;
    r = l.calculate(Module.createModules(['a/a', 'a/b']));
    var c = l.getComboUris(r);
    var js = c.js;
  ...
  it('should trunk uri by comboMaxUriLength automatically', function() {
    ...
    var r;
    r = l.calculate(Module.createModules(ret));
    var c = l.getComboUris(r);
    var cjs = c.js;
  ...
  it('should works for packages', function(done) {
    ...
    var mods = loader.calculate(Module.createModules(['tests/a']));
    var uris = loader.getComboUris(mods);
    var host = location.host;

    expect(uris.js[0].uri).to.be.equal(
      'http://' + host + '/tests/specs/combo/' + 'tests/??a.js,b.js,c.js',
    );
  ...
  it('works for not combo for specified packages', function() {
    ...
    var allMods = loader.calculate(Module.createModules(['timestamp/y']));

    var comboUris = loader.getComboUris(allMods);

    var jss = comboUris.js;

    expect(jss[0].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/y.js',
    );
    expect(jss[1].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/x.js',
    );
    expect(jss[2].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/z.js',
    );
  });
  ...
  it('optimize common prefix', function() {
    ...
    var loader = new mx.Loader.ComboLoader();
    var mods = loader.calculate(
      Module.createModules(['t/a/b/a', 't/a/b/b', 't/a/b/c']),
    );
    var uris = loader.getComboUris(mods);
    var host = location.host;
    expect(uris.js[0].uri).to.be.equal(
      'http://' + host + '/t/a/b/??a.js,b.js,c.js',
    );
  });
});
