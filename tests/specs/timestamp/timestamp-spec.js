describe('timestamp for individual module works', function() {
  ...
  it('works and avoid repeated loading', function(done) {
    ...
    r1.promise.then(function() {
      expect(
        mx.Loader.Utils.endsWith(mods['timestamp/x'].uri, 'b.js'),
      ).to.be.equal(true);
      expect(
        mx.Loader.Utils.endsWith(mods['timestamp/z'].uri, 'z.js'),
      ).to.be.equal(true);
  ...
  it('can be set later', function(done) {
    ...
    r1.promise.then(function() {
      expect(
        mx.Loader.Utils.endsWith(mods['timestamp/x'].uri, 'q.js'),
      ).to.be.equal(true);
      expect(
        mx.Loader.Utils.endsWith(mods['timestamp/z'].uri, 'z.js'),
      ).to.be.equal(true);
