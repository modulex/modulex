...
describe('timestamp for individual module works in combine mode', function() {
  ...
  it('works theoretically', function() {
    ...
    expect(comboUris.js[0].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/??y.js,x.js,z.js?t=a.js',
    );
  });
  ...
  it('works theoretically when package has no combo', function() {
    ...
    expect(jss[0].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/y.js?t=a.js',
    );
    expect(jss[1].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/x.js?t=b.js',
    );
    expect(jss[2].uri).to.be.equal(
      'http://' + host + '/tests/specs/timestamp/z.js?t=z.js',
    );
  });
